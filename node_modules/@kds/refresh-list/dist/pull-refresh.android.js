...
import { DefaultRefreshControl, PullState, PushingState, } from './constant';
...
export default class PullRefresh extends Component {
    constructor(props) {
        ...
        this.setRefreshNode = (refreshNode) => {
    ...
    shouldComponentUpdate(nextProps, nextState) {
        if (
        // @ts-ignore
        !Object.keys(nextState).every((key) => {
            return nextState[key] === this.state[key];
        })) {
            return true;
        }
        // @ts-ignore
        const isPropsSame = Object.keys(nextProps).every((key) => {
            if (key === 'children') {
                return true;
            }
    ...
    startRefresh(calledFromNative = true) {
        if (!calledFromNative) {
    ...
    render() {
        ...
        return (<RefreshLayout isContentScroll={this.props.isContentScroll !== false} refreshable={true} style={[styles.refreshLayout, this.props.refreshLayoutStyle]} ref={this.setRefreshNode.bind(this)}>
        <RefreshHeader style={[
        ...
        ]} viewHeight={viewHeight} onPushingState={this._onPushingState}>

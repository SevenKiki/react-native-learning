...
import com.kwai.kds.pulltorefresh.refresh.indicator.PtrIndicator;
import com.kwai.kds.pulltorefresh.refresh.util.PtrCLog;
...
public class PtrFrameLayout extends ReactViewGroup {
    ...
    public static final int MIN_MOVED_Y_DISTANCE = 1;
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mPtrIndicator.hasLeftStartPosition()) {
            movePos(PtrIndicator.POS_START - mPtrIndicator.getCurrentPosY());
    ...
    public boolean dispatchTouchEvent(MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                mPtrIndicator.onPressDown(e.getX(actionIndex), e.getY(actionIndex));
                return true;
            case MotionEvent.ACTION_POINTER_UP:
                if (mActivePointerId == e.getPointerId(actionIndex)) {
                    // 如果松开的是活动手指，则选择新的活动手指
                    final int newActionIndex = actionIndex == 0 ? 1 : 0;
                    mActivePointerId = e.getPointerId(newActionIndex);
                    mPtrIndicator.onPressDown(e.getX(newActionIndex), e.getY(newActionIndex));
                }
                return true;
            case MotionEvent.ACTION_DOWN:
                mActivePointerId = e.getPointerId(0);
                mHasSendCancelEvent = false;
                mPtrIndicator.onPressDown(e.getX(0), e.getY(0));
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                // 这里用TouchSlop判断时，轻微滑动会有滑动不连续的体验问题
                boolean reallyMovedY = Math.abs(offsetY) >= MIN_MOVED_Y_DISTANCE;
                boolean moveDown = offsetY > 0 && reallyMovedY;
                boolean moveUp = !moveDown && reallyMovedY;
    ...
    private void performRefreshComplete() {
        ...
        notifyUIRefreshComplete(false);
        if (mPtrUIHandlerHolder.hasHandler()) {
            mPtrUIHandlerHolder.onUIPositionChange(this, mPtrIndicator.isUnderTouch(), mStatus, mPtrIndicator);
        }
    }

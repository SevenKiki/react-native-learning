...
typedef void (^RNCleanupBlock)(void);

static inline void executeCleanupBlock (__strong RNCleanupBlock *block) {
    (*block)();
}

#define rn_word_concat_(A,B) A##B
#define rn_word_concat(A,B) \
rn_word_concat_(A, B)

#define rn_keywordify autoreleasepool {}
#define rn_onExit \
rn_keywordify \
__strong RNCleanupBlock rn_word_concat(exit_block_,__LINE__) __attribute__((cleanup(executeCleanupBlock), unused)) = ^
...
- (void)renderLayerTo:(CGContextRef)context rect:(CGRect)rect
{
    ...
    @rn_onExit{
        if (weakSelf.vectorEffect == kRNSVGVectorEffectNonScalingStroke && path) {
            CGPathRelease(path);
        }
    };

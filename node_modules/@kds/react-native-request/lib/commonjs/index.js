...
async function request(originOptions) {
  ...
  let resolve;
  let reject;
  const timeout = options.timeout || (_reactNative.Platform.OS === 'android' ? 10 : 15);
  let timer;
  let hasResult = false;
  ...
  if (options.businessName === 'web_merchant' && !/^https?:\/\//.test(options.url || '')) {
    ...
    if ((0, _utils.versionCompare)(appVersion, '9.10.40') === _utils.VersionIs.LessThan) {
      options.url = `https://app.kwaixiaodian.com/${(options.url || '').replace(/^\//, '')}`;
    }
  } // 兼容安卓 9.8.50 之前的 Content-Type


  if (options.headers && (options.headers['content-type'] || options.headers['Content-Type']) && _reactNative.Platform.OS === 'android') {
    const {
      headers: {
    ...
    if ((0, _utils.versionCompare)(appVersion, '9.8.50') === _utils.VersionIs.LessThan && mergedContentType.indexOf('application/json') !== -1) {
      if (contentType) {
  ...
  if (options.headers && options.headers['content-type'] && _reactNative.Platform.OS === 'ios') {
    const {
      headers: {
        'content-type': contentType
      }
    ...
    if ((0, _utils.versionCompare)(appVersion, '10.1.30') === _utils.VersionIs.LessThan) {
  ...
  const isIosLessThan10_5_10 = _reactNative.Platform.OS === 'ios' && (0, _utils.versionCompare)((0, _utils.getAppVersion)(), '10.5.10') === _utils.VersionIs.LessThan; // Android 11.0.30 以下且 responseType 等于 string 时不应该使用 V2 接口


  const isAndroidAndReponseTypeStringAndLessThan11_0_30 = options.responseType === 'string' && _reactNative.Platform.OS === 'android' && (0, _utils.versionCompare)((0, _utils.getAppVersion)(), '11.0.30') === _utils.VersionIs.LessThan;

...
const useCountDown = options => {
  const onEndRef = (0, _react.useRef)(options.onEnd);
  const updateTimeRef = (0, _react.useRef)(null);
  const updateServerTimeRef = (0, _react.useRef)(null);
  const {
    useServerTime
  } = options || {}; // 当前时间

  const nowTime = (0, _react.useRef)(Date.now());
  const endTime = (0, _react.useRef)(options.endTime);
  const [formattedRes, setFormattedRes] = (0, _react.useState)(() => {
    const {
      res

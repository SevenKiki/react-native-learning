...
void JReactMarker::setLogPerfMarkerIfNeeded() {
  ...
  std::call_once(flag, []() {
    ...
    ReactMarker::logMarkerWithUniqueId = JReactMarker::logMarkerWithUniqueId;
  });
}
...
void JReactMarker::logMarker(
  ...
  static auto cls = javaClassStatic();
  static auto meth =
      cls->getStaticMethod<void(std::string, std::string, int)>("logMarkerWithUniqueId");
  meth(cls, marker, tag, uniqueId);
}
#endif

void JReactMarker::logMarkerWithUniqueId(
...
#else
      JReactMarker::logMarker("RUN_JS_BUNDLE_START", tag, uniqueId);
#endif
      break;
...
#else
      JReactMarker::logMarker("RUN_JS_BUNDLE_END", tag, uniqueId);
#endif
      break;

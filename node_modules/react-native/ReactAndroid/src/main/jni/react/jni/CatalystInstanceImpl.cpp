...
class JInstanceCallback : public InstanceCallback {
 public:
  ...
  bool useCodeCache() override {
    jni::ThreadScope guard;
    static auto method =
        ReactCallback::javaClassStatic()->getMethod<jboolean()>("useCodeCache");
    return method(jobj_);
  };

  jsi::CodeCacheItem getValidatedCodeCache(int hash) override {
    jni::ThreadScope guard;
    static auto method = ReactCallback::javaClassStatic()
                             ->getMethod<jni::alias_ref<JByteBuffer>(jint)>(
                                 "getValidatedCodeCache");
    auto data = method(jobj_, hash);
    if (!data) {
      return {.data = nullptr,
              .size = 0,
              .result = jsi::CodeCacheItem::Result::NO_BUFFER};
    } else if (!data->isDirect()) {
      return {.data = nullptr,
              .size = 0,
              .result = jsi::CodeCacheItem::Result::NOT_DIRECT_BUFFER};
    } else {
      size_t bufferSize = data->getDirectSize();
      std::unique_ptr<uint8_t> buffer(new uint8_t[bufferSize]);
      memcpy(buffer.get(), data->getDirectBytes(), bufferSize);
      return {.data = std::move(buffer),
              .size = static_cast<int>(bufferSize),
              .result = jsi::CodeCacheItem::Result::INITIALIZED};
    }
  };

  void updateCodeCache(int hash, jsi::CodeCacheItem& codeCacheItem) override {
    jni::ThreadScope guard;
    local_ref<JByteBuffer> wrapper =
        JByteBuffer::wrapBytes(codeCacheItem.data.get(), codeCacheItem.size);
    static auto method =
        ReactCallback::javaClassStatic()
            ->getMethod<void(
                jint, jni::alias_ref<jni::JByteBuffer>, jint, jint)>(
                "updateCodeCache");
    method(jobj_, hash, wrapper, codeCacheItem.size, codeCacheItem.result);
  };
...
void CatalystInstanceImpl::registerNatives() {
  registerHybrid({
      ...
      makeNativeMethod(
          "jniLoadScriptFromAssetsWithKey",
          CatalystInstanceImpl::jniLoadScriptFromAssetsWithKey),
      makeNativeMethod(
          "jniLoadScriptFromFile", CatalystInstanceImpl::jniLoadScriptFromFile),
      makeNativeMethod(
          "jniLoadScriptFromFileWithKey",
          CatalystInstanceImpl::jniLoadScriptFromFileWithKey),
      makeNativeMethod(
...
void CatalystInstanceImpl::jniLoadScriptFromAssets(
  ...
  auto assetName = assetURL.substr(kAssetsLength);
  ...
  auto script = loadScriptFromAssets(manager, assetName);
  if (JniJSModulesUnbundle::isUnbundle(manager, assetName)) {
    auto bundle = JniJSModulesUnbundle::fromEntryFile(manager, assetName);
...
void CatalystInstanceImpl::jniLoadScriptFromAssetsWithKey(
    jni::alias_ref<JAssetManager::javaobject> assetManager,
    const std::string &assetURL,
    bool loadSynchronously,
...
void CatalystInstanceImpl::jniLoadScriptFromFileWithKey(
    const std::string &fileName,
    const std::string &sourceURL,
    bool loadSynchronously,
    const std::string &cacheKey) {

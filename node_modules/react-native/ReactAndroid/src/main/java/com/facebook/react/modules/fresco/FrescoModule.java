...
public class FrescoModule extends ReactContextBaseJavaModule
  ...
  public static void initializeAsyncWithDefaultConfig(Context context) {
    if (!hasBeenInitialized()) {
      new Thread(null, () -> {
        if (!hasBeenInitialized()) {
          sHasBeenInitialized = true;
          ReactContext reactContext = new ReactContext(context);
          ImagePipelineConfig config = getDefaultConfig(reactContext);

...
public class ReactImageView extends GenericDraweeView {
  ...
  public static final float IMAGE_SCALE_FACTOR_THRESHOLD = 3f;
  ...
  private boolean isDevSupportEnable = ReactFeatureFlags.enableReactImageWarning;
  ...
  public void maybeUpdateView() {
    ...
    float maxBitmapSize = BitmapUtil.MAX_BITMAP_SIZE;
    boolean resizingOverride = true;
    if (ReactFeatureFlags.enableResizeMethod) {
      maxBitmapSize = getWidth() * getHeight() * 4;
      maxBitmapSize = Math.max(maxBitmapSize, BitmapUtil.MAX_BITMAP_SIZE);
      resizingOverride = false;
    }

    ResizeOptions resizeOptions =
        doResize ? new ResizeOptions(getWidth(), getHeight(), maxBitmapSize) : null;
    ...
    ImageRequestBuilder imageRequestBuilder =
        ImageRequestBuilder.newBuilderWithSource(mImageSource.getUri())
            ...
            .setResizingAllowedOverride(resizingOverride)
    ...
    if (ReactBuildConfig.PROFILE) {
      imageRequestBuilder.setRequestListener(new BaseRequestListener() {
        ...
        public void onRequestStart(ImageRequest request, Object callerContext, String requestId, boolean isPrefetch) {
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(getId(), ImageLoadEvent.ON_REQUEST_START));
        }

        @Override
        public void onRequestSuccess(ImageRequest request, String requestId, boolean isPrefetch) {
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(getId(), ImageLoadEvent.ON_REQUEST_END));
        }
        ...
        public void onProducerStart(String requestId, String producerName) {
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(getId(), ImageLoadEvent.ON_DECODE_START));
        ...
        public void onProducerFinishWithSuccess(String requestId, String producerName, @javax.annotation.Nullable Map<String, String> extraMap) {
          ...
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(getId(), ImageLoadEvent.ON_DECODE_END));
        }

        @Override
        public void onProducerFinishWithFailure(String requestId, String producerName, Throwable t, @javax.annotation.Nullable Map<String, String> extraMap) {
          super.onProducerFinishWithFailure(requestId, producerName, t, extraMap);
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(getId(), ImageLoadEvent.ON_DECODE_END));
        }
      });
    }
    ...
    if (ReactBuildConfig.PROFILE) {
      ForwardingControllerListener combinedListener = new ForwardingControllerListener();
      combinedListener.addListener(mDraweeControllerBuilder.getControllerListener());
      combinedListener.addListener(new BaseControllerListener<ImageInfo>() {
        @Override
        public void onSubmit(String id, Object callerContext) {
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(getId(), ImageLoadEvent.ON_LOAD_START));
        ...
        public void onFinalImageSet(String id, @javax.annotation.Nullable ImageInfo imageInfo, @javax.annotation.Nullable Animatable animatable) {
          ...
          ProfileModule.dispatchEvent((ReactContext) getContext(), new ImageLoadEvent(
            getId(), ImageLoadEvent.ON_LOAD, mImageSource.getSource(), imageInfo.getWidth(), imageInfo.getHeight()));
        }
      });
      mDraweeControllerBuilder.setControllerListener(combinedListener);
    }
  ...
  private boolean shouldShowScaleWarn(float tWidth, float tHeight, float iWidth, float iHeight) {
    if (mScaleThreshold < 0) return false;
    if (iWidth > tWidth * mScaleThreshold) return true;

    return (iHeight > tHeight * mScaleThreshold);
  }

  private void checkImageScale(ImageInfo imageInfo) {
    if (!isDevSupportEnable) {
      FLog.d(TAG, "checkImageScale :: ignore for not debug");
      return;
    }
    ...
    if (shouldShowScaleWarn(targetW, targetH, imageW, imageH)) {
      String warningMsg = String.format("warn !!! ReactImageView(ID %s): \n" +
              ...
              "设置阈值",
          getId(), imageW, imageH, targetW, targetH, mScaleThreshold, uri);
      FLog.w(TAG, warningMsg);
      Toast.makeText(getContext(), warningMsg, Toast.LENGTH_LONG).show();
    }
  }

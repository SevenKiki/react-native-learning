...
public class UIImplementation {
  ...
  public final @Nullable ViewManager resolveViewManager(String className) {
  ...
  public void createView(int tag, String className, int rootViewTag, ReadableMap props) {
    synchronized (uiImplementationThreadLock) {
      ...
      if (rootNode == null) {
        FLog.w(ReactConstants.TAG, "Tried to createView non-existent root tag: " + rootViewTag);
        return;
      }
  ...
  public void updateView(int tag, String className, ReadableMap props) {
    ...
    if (cssNode == null) {
      FLog.e(ReactConstants.TAG, "Trying to update non-existent view with tag " + tag);
      return;
    }
  ...
  public void setChildren(int viewTag, ReadableArray childrenTags) {
    synchronized (uiImplementationThreadLock) {
      ...
      if (cssNodeToManage == null) {
        FLog.w(ReactConstants.TAG, "Tried to setChildren non-existent tag: " + viewTag);
        return;
      }
  ...
  public void removeChildren(int viewTag, ReadableArray childrenTags) {
    synchronized (uiImplementationThreadLock) {
      ReactShadowNode cssNodeToManage = mShadowNodeRegistry.getNode(viewTag);
      if (cssNodeToManage == null) {
        throw new IllegalViewOperationException("Trying to remove children from unknown view tag: " + viewTag);
      }

      ArrayList indicesToRemove = new ArrayList();
      for (int i = 0; i < childrenTags.size(); i++) {
        ReactShadowNode cssNodeToRemove = mShadowNodeRegistry.getNode(childrenTags.getInt(i));
        int index = cssNodeToManage.indexOf(cssNodeToRemove);
        if (index < 0) {
      ...
      manageChildren(viewTag, null, null, null, null, Arguments.makeNativeArray(indicesToRemove));
    }
  }
  ...
  protected void updateViewHierarchy() {
    ...
    try {
      for (int i = 0; i < mShadowNodeRegistry.getRootNodeCount(); i++) {
        ...
        if (cssRoot == null) {
          FLog.w(ReactConstants.TAG, "Tried to updateViewHierarchy non-existent root tag: " + tag);
          continue;
        }
  ...
  private void measureLayoutRelativeToParent(int tag, int[] outputBuffer) {
    ReactShadowNode node = mShadowNodeRegistry.getNode(tag);
    if (node == null) {
      FLog.e(ReactConstants.TAG, "No native view for tag " + tag + " exists!");
      return;
    }

...
#include "V8LiteExecutorFactory.h"
...
std::unique_ptr<Runtime> makeV8RuntimeSystraced(
  ...
  SystraceSection s("V8LiteExecutorFactory::makeV8RuntimeSystraced");
  return createV8Runtime(timezoneId, delegate, traceFilename, enableStartupTrace);
}

} // namespace

std::unique_ptr<JSExecutor> V8LiteExecutorFactory::createJSExecutor(
    std::shared_ptr<ExecutorDelegate> delegate,
  ...
  errorPrototype.setProperty(*v8Runtime, "jsEngine", "v8-lite");

  return std::make_unique<V8LiteExecutor>(
      std::move(v8Runtime),
      delegate,
      jsQueue,
      timeoutInvoker_,
...
bool V8LiteExecutor::encodeEvaluateParameters(std::string &sourceURL, int hash) {

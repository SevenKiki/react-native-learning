...
import com.krn.react.exception.KrnExceptionsManager;
...
public class ExceptionsManagerModule extends ReactContextBaseJavaModule {
  ...
  public void reportException(ReadableMap data) {
    ...
    if (mDevSupportManager.getDevSupportEnabled()) {
      ...
      String jsStackTrace = JSStackTrace.format(message, stack);

      KrnExceptionsManager.get()
        .dispatchException(getReactApplicationContext(), isFatal, jsStackTrace, extraDataAsJson);

      if (isFatal) {
        throw new JavascriptException(jsStackTrace)
          .setExtraDataAsJson(extraDataAsJson);
      } else {
        FLog.e(ReactConstants.TAG, jsStackTrace);

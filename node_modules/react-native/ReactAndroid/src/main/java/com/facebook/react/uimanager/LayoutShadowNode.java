...
import com.facebook.react.uimanager.kdscss.KdsUIImplementation;
...
public class LayoutShadowNode extends ReactShadowNodeImpl {
  ...
  private boolean mIsFixedNode = false;
  ...
  /**
   * 原来是fixed属性,position属性变了，从RootView返回原生层级逻辑
   */
  public void moveToOriginalParent() {
    if (getThemedContext() != null) {
      UIImplementation implementation =
        getThemedContext().getNativeModule(UIManagerModule.class).getUIImplementation();
      if (implementation instanceof KdsUIImplementation) {
        KdsUIImplementation kdsImplementation = (KdsUIImplementation) implementation;
        if (mIsFixedNode && getParent() != null &&
          kdsImplementation.isRootNode(getParent().getReactTag())) {
          kdsImplementation.moveFixedChildFromRootViewToOriginalParent(this);
        }
      }
    }
  }
  ...
  /**
   * 原来是非fixed属性，position变成fixed且已在view层级，将节点移到RootView下
   */
  public void moveToRootView() {
    if (getThemedContext() != null) {
      ...
      if (implementation instanceof KdsUIImplementation) {
        KdsUIImplementation kdsImplementation = (KdsUIImplementation) implementation;
        if (getParent() != null && !kdsImplementation.isRootNode(getParent().getReactTag())) {
          kdsImplementation.moveFixedChildFromParentToRootView(this, getParent());
        }
      }
    }
  }

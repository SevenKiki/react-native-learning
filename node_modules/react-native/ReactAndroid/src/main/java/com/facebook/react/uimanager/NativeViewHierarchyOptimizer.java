...
public class NativeViewHierarchyOptimizer {
  ...
  public void crossLevelMoveChild(ReactShadowNode child, ReactShadowNode destParent, int index) {
    ReactShadowNode nativeParent = child.getNativeParent();
    if (child == null || destParent == null || nativeParent == null) {
      return;
    }

    if (!ENABLED) {
      assertNodeSupportedWithoutOptimizer(child);
      mUIViewOperationQueue
        .crossLevelMoveChild(child.getReactTag(), destParent.getReactTag(), index);
      return;
    }

    int indexInNativeChildren = destParent.getNativeOffsetForChild(destParent.getChildAt(index));
    if (destParent.getNativeKind() != NativeKind.PARENT) {
      NodeIndexPair result = walkUpUntilNativeKindIsParent(destParent, indexInNativeChildren);
      if (result == null) {
        // If the parent hasn't been attached to its native parent yet, don't issue commands to the
        // native hierarchy. We'll do that when the parent node actually gets attached somewhere.
        return;
      }
      destParent = result.node;
      indexInNativeChildren = result.index;
    }

    nativeParent.removeNativeChildAt(nativeParent.indexOfNativeChild(child));
    destParent.addNativeChildAt(child, indexInNativeChildren);

    mUIViewOperationQueue
      .crossLevelMoveChild(child.getReactTag(), destParent.getReactTag(), indexInNativeChildren);
  }
}

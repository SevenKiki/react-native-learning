...
public final class NetworkingModule extends NativeNetworkingAndroidSpec {
  ...
  private static final String ATTACH_COMMON_PARAMS_HEADER_NAME = "attach_common_params";
  ...
  private final OkHttpClient mOriginClient;
  ...
  private final List<ResponseHandler> mResponseHandlers = new CopyOnWriteArrayList<>();
  ...
  /* package */ NetworkingModule(
    ...
    OkHttpClient.Builder originClientBuilder = OkHttpClientProvider.createOriginClientBuilder(reactContext);
    if (networkInterceptorCreators != null) {
      OkHttpClient.Builder clientBuilder = client.newBuilder();
      for (NetworkInterceptorCreator networkInterceptorCreator : networkInterceptorCreators) {
        clientBuilder.addNetworkInterceptor(networkInterceptorCreator.create());
        originClientBuilder.addNetworkInterceptor(networkInterceptorCreator.create());
      }
    ...
    originClientBuilder.cookieJar(mCookieJarContainer);
    mOriginClient = originClientBuilder.build();
    mShuttingDown = false;
  ...
  public void sendRequestInternal(
    ...
    String attachCommonParams = requestHeaders.get(ATTACH_COMMON_PARAMS_HEADER_NAME);
    if (TextUtils.equals(attachCommonParams, "false")) {
      oldClient = mOriginClient;
    } else {
      oldClient = mClient;
    }

    // 使用完直接删除掉
    requestHeaders = requestHeaders.newBuilder().removeAll(ATTACH_COMMON_PARAMS_HEADER_NAME).build();

    OkHttpClient.Builder clientBuilder = oldClient.newBuilder();
    ...
    if (timeout != oldClient.connectTimeoutMillis()) {
    ...
    if (data == null || method.toLowerCase().equals("get") || method.toLowerCase().equals("head")) {
      ...
      MultipartBody.Builder multipartBuilder =
          constructMultipartBody(parts, contentType, requestId, eventEmitter);
  ...
  private void cancelRequest(final int requestId) {
    ...
    new GuardedAsyncTask<Void, Void>(getReactApplicationContext()) {
      ...
      protected void doInBackgroundGuarded(Void... params) {
        ...
        OkHttpCallUtil.cancelTag(mOriginClient, Integer.valueOf(requestId));
      }
    }.execute();
  }
  ...
  private @Nullable MultipartBody.Builder constructMultipartBody(
      ReadableArray body, String contentType, int requestId, RCTDeviceEventEmitter emitter) {
    RCTDeviceEventEmitter eventEmitter = emitter;
  ...
  private RCTDeviceEventEmitter getEventEmitter(String reason) {
    ...
    if (reactApplicationContext != null) {
      TransientPageModule pageModule = getReactApplicationContext().hasNativeModule(TransientPageModule.class)
        ? getReactApplicationContext().getNativeModule(TransientPageModule.class)
        : null;
      if (pageModule != null && pageModule.isActive()) {
        return getReactApplicationContext().getJSModule(TransientEventEmitter.class);
      } else {

...
public class CoreModulesPackage extends TurboReactPackage implements ReactPackageLogger {
  ...
  public ReactModuleInfoProvider getReactModuleInfoProvider() {
    try {
      ...
      if (!ReactFeatureFlags.enableUiManagerOnTurboModule) {
        final ArrayList<Class<? extends NativeModule>> list =
          new ArrayList<>(Arrays.asList(moduleList));
        list.add(UIManagerModule.class);
        moduleList = list.toArray(moduleList);
      } else {
        reactModuleInfoMap.put(NativeUIManagerSpec.LIB_UIMANAGER_TURBO,
          ReactModuleInfo.fromClass(UIManagerTurboModule.class, true));
      }
  ...
  public NativeModule getModule(String name, ReactApplicationContext reactContext) {
    switch (name) {
      ...
      case ExceptionsManagerModule.NAME:
        return new ExceptionsManagerModule(reactContext, mReactInstanceManager.getDevSupportManager());
      ...
      case NativeUIManagerSpec.LIB_UIMANAGER:
      case NativeUIManagerSpec.LIB_UIMANAGER_TURBO:
  ...
  private NativeModule createUIManager(final ReactApplicationContext reactContext) {
        ...
        UIManagerModule.ViewManagerResolver resolver =
          new UIManagerModule.ViewManagerResolver() {
            ...
            public List<ViewManager> getFallbackViewManagers() {
              return mReactInstanceManager.getFallbackViewManagers();
            }
          };
        if (ReactFeatureFlags.enableUiManagerOnTurboModule) {
          return new UIManagerTurboModule(reactContext, resolver,
            mMinTimeLeftInFrameForNonBatchedOperationMs);
        } else {
          return new UIManagerBridgeModule(
            reactContext, resolver, mMinTimeLeftInFrameForNonBatchedOperationMs);
        }
      } else {
        if (ReactFeatureFlags.enableUiManagerOnTurboModule) {
          return new UIManagerTurboModule(reactContext,

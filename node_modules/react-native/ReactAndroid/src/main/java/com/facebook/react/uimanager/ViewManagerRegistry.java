...
public final class ViewManagerRegistry {
  ...
  private void initFallbackViewManager() {
    if (hasInitFallback || mViewManagerResolver == null) {
      return;
    }

    Systrace.beginSection(Systrace.TRACE_TAG_REACT_VIEW, "initFallbackViewManager");
    List<ViewManager> fallbackViewManagers = mViewManagerResolver.getFallbackViewManagers();
    for (ViewManager fallbackViewManager : fallbackViewManagers) {
      mViewManagers.put(fallbackViewManager.getName(), fallbackViewManager);
    }
    hasInitFallback = true;
  ...
  private @Nullable ViewManager getViewManagerFromResolver(String className) {
    ...
    } else if (!hasInitFallback) {
      initFallbackViewManager();

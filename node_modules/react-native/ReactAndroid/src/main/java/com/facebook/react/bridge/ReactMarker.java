...
public class ReactMarker {
  ...
  public interface TMMMarkerListener {
    void logTMMMarker(String marker, @Nullable String moduleName, @Nullable String methodName, int id);
  };
  ...
  @DoNotStrip
  public static void logTMMMarker(String marker, @Nullable String moduleName, @Nullable String methodName, int id) {
    for (MarkerListener listener : sListeners) {
      if (listener instanceof TMMMarkerListener) {
        ((TMMMarkerListener)listener).logTMMMarker(marker, moduleName, methodName, id);
      }
    }
  }

  public interface DirectionalMarkerListener {
    void directionalLogMarker(ReactMarkerConstants name, @Nullable String tag, int uniqueId);
  };

  private static final List<DirectionalMarkerListener> sDirectionalListeners =
      new CopyOnWriteArrayList<>();

  @DoNotStrip
  public static void addDirectionalMarkerListener(DirectionalMarkerListener listener) {
    if (!sDirectionalListeners.contains(listener)) {
      sDirectionalListeners.add(listener);
    }
  }

  @DoNotStrip
  public static void removeDirectionalMarkerListener(DirectionalMarkerListener listener) {
    sDirectionalListeners.remove(listener);
  }
  ...
  @DoNotStrip
  public static void logMarkerWithUniqueId(String name, @Nullable String tag, int uniqueId) {
    ReactMarkerConstants marker = ReactMarkerConstants.valueOf(name);
    logMarker(marker, tag, 0);
    directionalLogMarker(marker, tag, uniqueId);
  }

  public static void directionalLogMarker(ReactMarkerConstants name, @Nullable String tag,
      int uniqueId) {
    for (DirectionalMarkerListener listener : sDirectionalListeners) {
      listener.directionalLogMarker(name, tag, uniqueId);
    }
  }
}

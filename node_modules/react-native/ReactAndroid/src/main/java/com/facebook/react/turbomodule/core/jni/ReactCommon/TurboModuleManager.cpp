...
bool TurboModuleManager::callFunction(
    const std::string &method,
    NativeArray *arguments) {
  if (!runtime_ || !jsCallInvoker_) {
    return false; // Runtime doesn't exist when attached to Chrome debugger.
  }

  if (!turboCallAndReturn_) {
    jsi::Value turboCallAndReturnValue =
        runtime_->global().getProperty(*runtime_, "__turboCallAndReturn");
    if (turboCallAndReturnValue.isUndefined() || !turboCallAndReturnValue.isObject()) {
      return false;
    }

    jsi::Object turboCallAndReturnObject = turboCallAndReturnValue.getObject(*runtime_);
    if (!turboCallAndReturnObject.isFunction(*runtime_)) {
      return false;
    }

    turboCallAndReturn_ = turboCallAndReturnObject.asFunction(*runtime_);
  }
  ...
  folly::dynamic params = arguments->consume();
  jsCallInvoker_->invokeAsyncBridgeless(
      [this, method = method, params = std::move(params)]() -> void {
        jsi::Value ret = jsi::Value::undefined();
        ret = turboCallAndReturn_->call(*runtime_, method, jsi::valueFromDynamic(*runtime_, params));
      });
...
void TurboModuleManager::registerNatives() {
  registerHybrid({
      ...
      makeNativeMethod(
          "callFunction", TurboModuleManager::callFunction),
      makeNativeMethod(

...
public abstract class ReactPackageTurboModuleManagerDelegate extends TurboModuleManagerDelegate {
  protected final List<TurboReactPackage> mPackages = new ArrayList<>();
  ...
  private TurboModule resolveModule(String moduleName) {
    ...
    for (final TurboReactPackage pkg : mPackages) {
      ...
      try {
        final ReactModuleInfo info =
          pkg.getReactModuleInfoProvider().getReactModuleInfos().get(moduleName);
        if (info == null || !info.isTurboModule()) {
          if (ReactFeatureFlags.tempFixTurboModuleCrash) {
            continue;
          } else {
            return null;
          }

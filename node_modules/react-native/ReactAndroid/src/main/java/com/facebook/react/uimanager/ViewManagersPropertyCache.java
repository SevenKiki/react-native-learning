...
/*package*/ class ViewManagersPropertyCache {
  ...
  /*package*/ abstract static class PropSetter {
    ...
    // KDS BEGIN
    // only run on main thread, it is ok
    private static final Object[] VIEW_MGR_ARGS = new Object[2];
    private static final Object[] VIEW_MGR_GROUP_ARGS = new Object[3];

    // may run on multi mqt_native_modules threads, it is not ok
    private static ThreadLocal<Object[]> SHADOW_ARGS = new ThreadLocal<Object[]>() {
      @Nullable
      @Override
    ...
    private static ThreadLocal<Object[]> SHADOW_GROUP_ARGS = new ThreadLocal<Object[]>() {
      @Nullable
      @Override
      protected Object[] initialValue() {
        return new Object[2];
...
//    private static final Object[] SHADOW_GROUP_ARGS = new Object[2];
    ...
    public void updateShadowNodeProp(ReactShadowNode nodeToUpdate, Object value) {
      try {
        // KDS BEGIN
        if (mIndex == null) {
          Object[] shadowArgs = SHADOW_ARGS.get();

          shadowArgs[0] = getValueOrDefault(value);
          mSetter.invoke(nodeToUpdate, shadowArgs);
          Arrays.fill(shadowArgs, null);
        } else {
          Object[] shadowGroupArgs = SHADOW_GROUP_ARGS.get();

          shadowGroupArgs[0] = mIndex;
          shadowGroupArgs[1] = getValueOrDefault(value);
          mSetter.invoke(nodeToUpdate, shadowGroupArgs);
          Arrays.fill(shadowGroupArgs, null);
        }
        // KDS_END
      } catch (Throwable t) {
        FLog.e(ViewManager.class, "Error while updating prop " + mPropName, t);

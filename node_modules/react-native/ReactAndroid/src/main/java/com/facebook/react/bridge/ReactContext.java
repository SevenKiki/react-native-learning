...
public class ReactContext extends ContextWrapper {
  ...
  private final CopyOnWriteArraySet<WindowFocusChangeListener> mWindowFocusEventListeners =
      new CopyOnWriteArraySet<>();
  private final CopyOnWriteArraySet<ScrollEventListener> mScrollEventListeners =
      new CopyOnWriteArraySet<>();
  ...
  private @Nullable
  NativeModuleCallExceptionHandler mExceptionHandlerWrapper;
  private @Nullable
  WeakReference<Activity> mCurrentActivity;
  private FpsListener mFpsListener = new FpsListener() {
    @Override
    public void enable(String tag) {
      onScrollEvent(true, tag);
    }

    @Override
    public void disable(String tag) {
      onScrollEvent(false, tag);
    }

    @Override
    public boolean isEnabled() {
      return ReactFeatureFlags.enableScrollEventListener.get();
    }
  };
  ...
  public void removeScrollEventListener(ScrollEventListener listener) {
    mScrollEventListeners.remove(listener);
  }
  ...
  @UiThread
  public void onScrollEvent(boolean isBegin, String tag) {
    for (ScrollEventListener listener : mScrollEventListeners) {
      try {
        if (isBegin) {
          listener.onScrollBegin(tag);
        } else {
          listener.onScrollEnd(tag);
        }
      } catch (RuntimeException e) {
        handleException(e);
      }

...
@interface RCTBackedTextFieldDelegateAdapter () <UITextFieldDelegate, RCTUITextFieldSupplementaryDelegate>
@end
...
- (instancetype)initWithTextField:(UITextField<RCTBackedTextInputViewProtocol> *)backedTextInputView
{
  if (self = [super init]) {
    ...
    if ([backedTextInputView isKindOfClass:[RCTUITextField class]]) {
      ((RCTUITextField *)backedTextInputView).supplementaryDelegate = self;
    }
...
- (void)textFieldDeleteBackward:(UITextField *)textField {
    // 当 textField 为空的时候，点击退格键不会回调 shouldChangeCharactersInRange ，跟 UITextView 表现不一致，作此修复
    if (!_lastText.length) {

...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _reactSubviews = [NSMutableArray array];
      ...
      _originalParentTag = @(-1);
      _isBindingVirtualNode = NO;
      _moveOutFixedChildrenTags = [[NSMutableArray alloc] init];
...
- (void)setPosition:(NSInteger)position
{
    if (RCTKDSFixedEnabled()) {
        // 0 : YGPositionType.YGPositionTypeRelative
        // 1 : YGPositionType.YGPositionTypeAbsolute
        // 2 : 新增，用于支持fixed属性，但未在yoga中声明该枚举值。另外，fixed属性，目前会先复用yoga的absolute实现。
        switch (position) {
                // 对应前端的 absolute
            case (NSInteger)YGPositionTypeAbsolute:
                [self moveSelfToOriginalParent];
                YGNodeStyleSetPositionType(_yogaNode, YGPositionTypeAbsolute);
                _isFixedNode = NO;
                break;
                // 对应前端的 fixed
            case YGPositionTypeFixed:
                [self moveSelfToRootView];
                YGNodeStyleSetPositionType(_yogaNode, YGPositionTypeAbsolute); // fixed属性，复用yoga的absolute实现
                _isFixedNode = YES;
                break;
                // 对应前端的 relative
            case (NSInteger)YGPositionTypeRelative:
                // 默认值：relative
            default:
                [self moveSelfToOriginalParent];
                YGNodeStyleSetPositionType(_yogaNode, YGPositionTypeRelative);
                _isFixedNode = NO;
                break;
        }
    } else {
...
// 原来是fixed节点,position属性变了，从RootView返回原生层级逻辑，并删除virtual View
- (void)moveSelfToOriginalParent
{
    if (_bridge.uiManager && _isFixedNode && self.reactSuperview && self.reactSuperview.isReactRootView == YES) {
        [_bridge.uiManager moveFixedChildFromRootViewToOriginalParent:self shadowViewRegistry:_bridge.uiManager.mShadowViewRegistry];
    }
}

// 原来是非fixed节点，position变成fixed且已在view层级，将节点移到RootView下,并生成virtual View
- (void)moveSelfToRootView
{
    if (_bridge.uiManager && self.reactSuperview && self.reactSuperview.isReactRootView == NO) {
        [_bridge.uiManager moveFixedChildFromParentToRootView:self parent:self.reactSuperview shadowViewRegistry:_bridge.uiManager.mShadowViewRegistry];
    }
}

- (void)addMovedOutFixedChildTag:(NSNumber *)tag;
{
    if (!_moveOutFixedChildrenTags) {
        _moveOutFixedChildrenTags = [[NSMutableArray alloc] init];
    }
    if (tag) {
        [_moveOutFixedChildrenTags addObject:tag];
    }
}

- (NSArray<NSNumber *> *)mGetMovedOutFixedChildTags
{
    return [_moveOutFixedChildrenTags copy];
}

- (void)resetMovedOutFixedChildTags:(NSMutableArray<NSNumber *> *)tags
{
    _moveOutFixedChildrenTags = tags;
}

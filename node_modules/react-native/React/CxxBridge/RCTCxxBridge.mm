...
public:
  ...
  std::unique_ptr<JSExecutor> createJSExecutor(
    ...
    auto ret = factory_->createJSExecutor(delegate, jsQueue, uniqueId);
...
static void registerPerformanceLoggerHooks(RCTPerformanceLogger *performanceLogger) {
  ...
  ReactMarker::logTaggedMarker = [weakPerformanceLogger](const ReactMarker::ReactMarkerId markerId, const char *__unused tag) {
    ...
    if (RCTKDSSdkLaunchTimeReportdEnabled()) {
        ReactMarker::logMarkerWithUniqueId = [weakPerformanceLogger](const ReactMarker::ReactMarkerId markerId, const char *__unused tag, const int uniqueId) {
            switch (markerId) {
                case ReactMarker::RUN_JS_BUNDLE_START:
                    KDSNotifyAboutRunJSBundleStart(weakPerformanceLogger, tag, uniqueId);
                    break;
                case ReactMarker::RUN_JS_BUNDLE_STOP:
                    KDSNotifyAboutRunJSBundleEnd(weakPerformanceLogger, tag, uniqueId);
                    break;
                default:
                    break;
            }
...
@implementation RCTCxxBridge
{
  ...
  NSRecursiveLock* _startDestroyLock;
}
...
- (instancetype)initWithParentBridge:(RCTBridge *)bridge
{
  ...
  if ((self = [super initWithDelegate:bridge.delegate
    ...
    _startDestroyLock = [NSRecursiveLock new];
...
- (void)start
{
  ...
  if (RCTTurboModuleEnabled() &&
      ...
      [self.delegate respondsToSelector:@selector(turboModuleRegistryForBridge:)]) {
    id<RCTTurboModuleRegistry> turboModuleRegistry = [self.delegate turboModuleRegistryForBridge:self];
    [self setRCTTurboModuleRegistry:turboModuleRegistry];
  }
  ...
  (void)[self _initializeModules:(NSArray<id<RCTBridgeModule>> *)RCTGetModuleClasses() withDispatchGroup:prepareBridge lazilyDiscovered:NO];
...
{
  RCTModuleData* data = nil;
  data = [self _safeGetModuldDataWithModuleName:moduleName];
  return data;
}
...
- (id)moduleForName:(NSString *)moduleName lazilyLoadIfNecessary:(BOOL)lazilyLoad
{
  ...
  if (!lazilyLoad) {
      return [self _safeGetModuldDataWithModuleName:moduleName].instance;
  }

  RCTModuleData *moduleData = [self _safeGetModuldDataWithModuleName:moduleName];
  if (moduleData) {
  ...
  if (result) {
    // Try again.
    moduleData = [self _safeGetModuldDataWithModuleName:moduleName];
...
- (BOOL)moduleIsInitialized:(Class)moduleClass
{
  ...
  if ([self _safeGetModuldDataWithModuleName:moduleName].hasInstance) {
    return YES;
  }
...
- (void)updateModuleWithInstance:(id<RCTBridgeModule>)instance
{
  ...
  if (moduleName) {
    RCTModuleData *const moduleData = [self _safeGetModuldDataWithModuleName:moduleName];
    if (moduleData) {
                                        ...
                                        lazilyDiscovered:(BOOL)lazilyDiscovered
{
  ...
  for (Class moduleClass in moduleClassesCopy) {
    if (RCTShouldEnableTurboModuleForClass(moduleClass)) {
      continue;
    }
    NSString *moduleName = RCTBridgeModuleNameForClass(moduleClass);
    ...
    [self _safeSetModuldData:moduleData moduleName:moduleName];
                                ...
                                lazilyDiscovered:(BOOL)lazilyDiscovered
{
  ...
  NSArray<RCTModuleData *> *moduleDataById = [self _registerModulesForClasses:(NSArray<Class> *)modules lazilyDiscovered:lazilyDiscovered];
...
- (void)registerAdditionalModuleClasses:(NSArray<Class> *)modules
{
  ...
  if (_moduleRegistryCreated) {
    NSArray<RCTModuleData *> *newModules = [self _initializeModules:(NSArray<id<RCTBridgeModule>> *)modules withDispatchGroup:NULL lazilyDiscovered:YES];
...
- (void)executeSourceCode:(NSData *)sourceCode sync:(BOOL)sync
{
  ...
  //Prevent reactinstance being released when executeSourceCode running.
  [_startDestroyLock lock];
  if (sync) {
                           ...
                           async:(BOOL)async
{
  [self _tryAndHandleError:^{
    ...
    sourceUrlStr = self.sourceUrlPath ? : sourceUrlStr;
    [[NSNotificationCenter defaultCenter]
...
- (void)batchDidComplete
{
  ...
  } queue:self.uiManager.methodQueue];
}
...
#pragma mark - safe moduleName

- (RCTModuleData *)_safeGetModuldDataWithModuleName:(NSString *)moduleName {
    RCTModuleData* data = nil;
    @synchronized (_moduleDataByName) {
        data = _moduleDataByName[moduleName];
    }
    return data;
}

- (void)_safeSetModuldData:(RCTModuleData *)data moduleName:(NSString *)moduleName {
    @synchronized (_moduleDataByName) {
        _moduleDataByName[moduleName] = data;
    }
}

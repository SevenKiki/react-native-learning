...
import { AssetResolver } from '../plugins';
...
export async function reactNativeAssetsLoader(this: any, contents: any) {
  this.cacheable();
  ...
  try {
    ...
    assets.forEach((asset) => {
      ...
      this.emitFile(options.assetDest ? path.join(options.assetDest, destination) : destination, content ?? '');
    });
    ...
    const hashes = await Promise.all(
      assets.map((asset) =>
        hasha.async(Buffer.from(asset.content || '') ?? asset.destination, {
          algorithm: 'md5',
        }),
      ),
    );
...
export default reactNativeAssetsLoader;

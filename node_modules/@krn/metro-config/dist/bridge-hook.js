            ...
            factory = function overrideFactory(...factoryArgs) {
                ...
                factoryArgs[4] = new Proxy(moduleObject, {
                    set: function (target, prop, moduleImpl) {
                        if (prop === 'exports') {
                            target[prop] = callback(path, moduleObject, moduleImpl);
                        }
                        return true;
                    },
                });
                return originalFactory.call(originalFactory, ...factoryArgs);
            };
        });
...
const emitInvoke = function (callbackId, bridgeType, namespace, methodName, params, multiParams) {
...
Hook(Object.values(moduleMap), function callback(modulePath, moduleObject, moduleImpl) {
    ...
    if (moduleMap.NativeEventEmitter === modulePath) {
        ...
        moduleImpl.prototype.addListener = function (eventType, listener, context) {
            ...
            emitInvoke(callbackId, 'Callback', 'eventEmitter', 'addListener', { eventType }, false);
            return originalAddListener.call(this, eventType, function (...args) {
                ...
                get: function (obj, prop) {
                    ...
                    if (isFunction(nmsModule)) {
                        ...
                        nmsModule = function (...args) {
                            ...
                            const hasCallback = args.some((a) => isFunction(a));
                            // promise规范的bridge接口
                            ...
                            // 上报bridge调用
                            emitInvoke(callbackId, bridgeType, lastProp, prop, requestParams, isMultipleParams);
                            if (isPromise) {
                                return originFunc
                                    .call(originFunc, ...args)
                                    .then((res) => {
                                    emitCallback(callbackId, bridgeType, res);
                                    return Promise.resolve(res);
                                })
                                    .catch((err) => {
                                    emitCallback(callbackId, bridgeType, null, err);
                            ...
                            // 一般的bridge接口
                                ...
                                if (hasCallback) {
                                    args.forEach((arg, index) => {
                                        if (isFunction(arg)) {
                                            const originalCallback = args[index];
                                            args[index] = function (...callbackArgs) {
                                                emitCallback(callbackId, bridgeType, callbackArgs);
                                                return originalCallback.call(originalCallback, ...callbackArgs);
                                            };
                                        }
                                        return arg;
                                    });
                                ...
                                try {
                                    const res = originFunc.call(originFunc, ...args);
                                    if (!hasCallback) {
                                        emitCallback(callbackId, bridgeType, res);
                                    }
                                    return res;

...
export type TIOSThemeChangeListener = (theme: ETheme) => void;
export type TIOSThemeChangeListenerRemoveFn = () => void;
/**
 * 获取当前主题模式
...
export function getTheme(): ETheme {
    ...
    try {
        theme = NativeModules.KSAppearance.getColorScheme() as ETheme;
    } catch (e) {
        // @ts-ignore
 ...
 * @return 移除监听函数
 */
export function addIOSThemeChange(
    listener: TIOSThemeChangeListener,
): TIOSThemeChangeListenerRemoveFn {
    if (Platform.OS === 'ios') {
...
export function getLanguage(): ELanguageType {
    let lan: ELanguageType = ELanguageType.ZH_HANS;
    try {
        lan = NativeModules.KSLanguage.getLanguage() as ELanguageType;
    } catch (e) {
        // @ts-ignore

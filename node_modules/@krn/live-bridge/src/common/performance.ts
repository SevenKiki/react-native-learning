 ...
 * @param extra
 */
export function reportAppDidMount(extra?: object) {
    const { rootTag } = getInitData();
    try {
        if (
            typeof NativeModules?.RootViewPageBridge?.rootViewDidMountWith !==
            'function'
        ) {
            throw new Error(
                'NativeModules.RootViewPageBridge.rootViewDidMountWith not function',
            );
        } else {
 ...
 */
export function reportPageRenderTime(
    fetchDoneTime: number,
    didUpdateTime: number,
    extra?: object,
): void {
    const { rootTag } = getInitData();
    ...
    try {
        if (
            typeof NativeModules?.RootViewPageBridge?.reportPageRenderTime !==
            'function'
        ) {
            throw new Error(
                'NativeModules.RootViewPageBridge.reportPageRenderTime not function',
            );
        } else {

...
class Start extends command_2.default {
    constructor() {
        super(...arguments);
    ...
    async runStart() {
        ...
        const termialReporter = path.resolve(__dirname, '../patch/metro/src/lib/TerminalReporter.js');
        ...
        process.env.KRN_CONFIG_PATH =
            flags.config || path.resolve('./metro.config.js');
        const cliArgs = [
            reactNativeCliPath,
            'start',
        ...
        const cliProcess = (0, child_process_1.spawn)('node', cliArgs, {
            stdio: 'inherit',
            env: Object.assign({ KRN_ENV: 'DEBUG', KRN_SCHEME_NAME: krnConfig.scheme, KRN_SCHEME_URL: schemeUrl, KRN_SCHEME_TEMPLATE: krnConfig.schemeTemplate, KRN_SCHEME_EXTRA_PARAMS: (0, lodash_1.isString)(krnConfig.schemeParams)
                    ? krnConfig.schemeParams
                    : (0, lodash_1.isPlainObject)(krnConfig.schemeParams)
                        ? krnConfig.schemeParams[componentName || 'unknown']
                        : '', KRN_IP: ip, KRN_DEVTOOLS_ENABLED: isDevtoolsEnabled }, process.env),
        });
        (_a = cliProcess.stderr) === null || _a === void 0 ? void 0 : _a.on('data', (data) => {
            const stderr = data.toString();
            utils_1.Logger.error(`Metro Sever Error: ${stderr}.`);
        });
    }
    async run() {
        try {
...
Start.hidden = false;
Start.description = '调试模式';
Start.examples = [`krn start`];
Start.flags = {

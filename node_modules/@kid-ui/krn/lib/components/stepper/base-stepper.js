...
export var StepperChangeType;
(function (StepperChangeType) {
    StepperChangeType[StepperChangeType["STATE_CHANGE_INC"] = 1] = "STATE_CHANGE_INC";
    StepperChangeType[StepperChangeType["STATE_CHANGE_DEC"] = 2] = "STATE_CHANGE_DEC";
    StepperChangeType[StepperChangeType["STATE_CHANGE_DIRECTLY"] = 3] = "STATE_CHANGE_DIRECTLY";
})(StepperChangeType || (StepperChangeType = {}));
...
const BaseStepper = (props) => {
    const { children, defaultValue = 0, value, max, min, step = 1, editable = true, onChange, countReducer } = props;
    ...
    const changeCount = (stateModifier, type) => {
    ...
    const setDirectly = (count) => {
        changeCount(() => {
            return count;
        }, StepperChangeType.STATE_CHANGE_DIRECTLY);
    };
    return (<StepperContext.Provider value={Object.assign({ count, increase, decrease, setDirectly }, props)}>
      {children}
    </StepperContext.Provider>);
};
const CountDecrease = () => {
    const { count, decrease, decreaseButtonStyle, disabled, min } = React.useContext(StepperContext);
    return (<TouchableOpacity testID='DecreaseBtn' hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} disabled={disabled || count === min} style={styles.decreaseContainer} activeOpacity={1} onPress={decrease}>
...
const CountIncrease = () => {
    ...
    return (<TouchableOpacity testID='IncreaseBtn' hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }} disabled={disabled || count === max} style={styles.increaseContainer} activeOpacity={1} onPress={increase}>
...
const CountContent = () => {
    ...
    return (<View style={[styles.content, { opacity: disabled ? 0.5 : 1 }, Platform.OS === 'web' ? { width: width > 36 ? width : 36 } : null, contentStyle]}>

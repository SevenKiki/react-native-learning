...
import { View, Text, PanResponder, FlatList, TouchableOpacity, NativeModules } from 'react-native';
const TRIANGLE_LEFT = 0.5 / Math.sqrt(2) + 0.25;
export const Indexes = (props) => {
    const { indexNum = 0, onChange = () => { }, indicatorBgColor = '#c6c6c6', indicatorTextColor = '#ffffff', indicatorTextFont = 30, indicatorHeight = 50, indicatorRightMargin = 40, indicatorCornerRadius = indicatorHeight / 2, //10
    indexBgColor = '#00000000', indexTextColor = '#666666', indexSelectedBgColor = '#FF5000', indexSelectedTextColor = '#ffffff', indexHeight = 18, indexRightMargin = 5, indexSpace = 3, indexTextFont = 11, indexSelectedRadius = 15, // 7
    indexTextStyle = {}, data = [
        'A',
        'B',
        'C',
    ...
    const indicatorWidth = indicatorRightMargin + (indicatorHeight * Math.sqrt(2)) / 2;
    ...
    function getWordIndex(y) {
        const index = Math.floor((y - indicatorHeight / 2 - top.current) / itemHeight.current);
        return {
            index,
        };
    }
    ...
    const _onLayout = (e) => {
        var _a, _b;
        (_b = (_a = NativeModules.UIManager) === null || _a === void 0 ? void 0 : _a.measure) === null || _b === void 0 ? void 0 : _b.call(_a, e.nativeEvent.target, (x, y, width, _height, pageX, pageY) => {
            top.current = pageY;
    ...
    return (<View style={{
        ...
        }} onLayout={_onLayout} {...panResponder.panHandlers}>
            <FlatList ListFooterComponent={<View style={{ height: indicatorHeight / 2 }}/>} ListHeaderComponent={<View style={{ height: indicatorHeight / 2 }}/>} data={data} keyExtractor={(item, index) => index.toString()} renderItem={renderItem}/>
        </View>);
};

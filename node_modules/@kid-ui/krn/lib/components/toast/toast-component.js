...
export class ToastComponent extends Component {
    ...
    onPropsChange(nextProps) {
        ...
        this.offset.setValue(percent > 100 ? 0 : ((100 - percent) / 100) * 14 * Math.PI);
    }
    ...
    render() {
        ...
        return (<Svg width={size} height={size} viewBox="0 0 38 38" style={style}>
                <Circle cx="19" cy="19" r="18" strokeWidth="2" stroke={type}/>
                <AnimateCircle cx="19" cy="19" r="7" fill="none" strokeWidth="14" stroke={type} strokeDasharray={14 * Math.PI} strokeDashoffset={this.offset} transform="rotate(-90, 19, 19)"/>
            </Svg>);
    }
}
export const KidToastComponent = (toast = {}) => {
    const { rem } = React.useContext(ScaleContext);
    const { toastStyle = {}, text = '', textStyle = {}, kid = 'style_toast_text', percent = 0, getPercentText } = toast;
    ...
    return (<View style={[
            ...
            {kid === 'style_toast_text' || '' ? (<Text style={[styles.toastText, { width: textWidth }, textStyle]} numberOfLines={textNumberOfLines}>{text}</Text>) : null}
            {kid === 'style_toast_success' && (<View style={{
                width: rem(120),
                height: rem(120),
                borderRadius: rem(8),
                ...
                alignItems: 'center',
                    ...
                    <KidIcon kid="common_toast_success" kidConfig={common_toast_success} width={38} height={29}/>
                    <Text style={[styles.toastSmallText, { marginTop: rem(16) }, textStyle]}>
                        {text}
                    </Text>
                </View>)}
            {kid === 'style_toast_failed' && (<View style={{
                width: rem(120),
                height: rem(120),
                borderRadius: rem(8),
                justifyContent: 'center',
                alignItems: 'center',
                    ...
                    <KidIcon kid="common_toast_error" kidConfig={common_toast_error} width={38} height={29}/>
                    <Text style={[styles.toastSmallText, { marginTop: rem(14) }, textStyle]}>
                        {text}
                    </Text>
                </View>)}
            {kid === 'style_toast_download' && (<View style={{
                width: rem(120),
                height: rem(120),
                borderRadius: rem(8),
                justifyContent: 'center',

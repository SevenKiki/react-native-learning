//@ts-nocheck
import { Dimensions, Animated, Keyboard, StyleSheet, Modal, Platform, TouchableWithoutFeedback, BackHandler } from 'react-native';
...
const { width: windowWidth, height: windowHeight } = Dimensions.get(Platform.OS === 'web' ? 'window' : 'screen');
const animationIn = (val1, val2, time, callback) => {
    Animated.parallel([
...
const animationOut = (val1, val2, time, callback = () => { }) => {
    Animated.parallel([
        Animated.timing(val1, {
            toValue: 0,
            duration: time,
...
export const WrapperComponent = function (props) {
    const { maskBgColor = 'rgb(111,111,111)', componentApi = true, useModal = true, visible = true, showMask = true, maskClosable = true, position = 'center', top = 0, left = 0, borderRadius = rem(12), popupStyle = {}, beforeClose, 
    // TODO: animate 重构成 {type: string; duration: number}
    animate = 'fade', animateDuration = animate === 'none' ? 0 : 300, isControlAnimationIn = false, children, onAnimateEnd = () => { }, disableGeastureBack = false, onShow = () => { }, onDismiss = () => { } } = props;
    const [innerVisible, setInnerVisible] = useState(false);
    // 动画部分
    let initSliderH = 1, initSliderV = 1, initFade = 0, animateType = animate;
    switch (animateType) {
        case 'sliderLeft':
            initSliderH = 0;
            break;
    ...
    const sliderOut = useCallback((time = animateDuration, animateType = animate) => {
        stopAnimation();
        switch (animateType) {
            case 'slider':
                switch (position) {
    ...
    let _left = '50%', _top = '50%', bottom, marginTop = -contentH / 2, marginLeft = -contentW / 2;
    switch (position) {
        case 'center':
    ...
    const renderContent = () => {
        return (<>
                {showMask && (<TouchableWithoutFeedback onPress={() => {
                    if (keyboardUp) {
                        Keyboard.dismiss();
                    }
    ...
    return (<>
            {showMask && useModal ? (<Modal onRequestClose={() => {
                if (!beforeClose) {
                    !componentApi && sliderOut();
                }
                else {
            ...
            }} visible={true} transparent={true} statusBarTranslucent={true}>
                    <RootSiblingParent inactive={true}>{renderContent()}</RootSiblingParent>
                </Modal>) : componentApi ? (<RootSiblingPortal>{renderContent()}</RootSiblingPortal>) : (renderContent())}
        </>);
};

...
export class ScrollPicker extends React.Component {
    constructor(props) {
        ...
        this.scrollers = [];
        this.mountScrollTime = null;
        this.willReceivePropsScrollTime = null;
        this.containerRef = null;
        const data = this.initialize(props);
        this.state = Object.assign(Object.assign({}, data), { isMomentumScroll: false });
        this.scrollers = [];
    }
    initialize(props) {
    ...
    scrollTo(scrollIndex, targetItemIndex, animated = false) {
        this.scrollProper(scrollIndex, ITEM_HEIGHT * targetItemIndex, animated);
    }
    ...
    scrollProper(scrollIndex, scrollHeight, animated) {
        ...
        if (scrollHeight <= min) {
            ...
            const quotient = parseInt(String(scrollHeight / ITEM_HEIGHT), 10);
    ...
    render() {
        const styles = scrollpickerStyles;
        const { list, proportion } = this.state;
        return (<View ref={(el) => {
                ...
                return (<View key={scrollIndex} style={[
                        styles.proportionWrapper,
                        { flex: Number(proportion[scrollIndex]) },
                    ]}>
                            <ScrollView nestedScrollEnabled ref={(c) => {
                        this.scrollers[scrollIndex] = c;
                    }} scrollEventThrottle={16} style={styles.scroller} showsVerticalScrollIndicator={false} contentContainerStyle={[
                        ...
                        return (<View key={index} style={[
                                ...
                                { height: ITEM_HEIGHT },
                                            ...
                                            {this.props.renderItem ? (this.props.renderItem(item, index)) : (<Text style={[

...
import { View, Text, TouchableOpacity, Animated, Easing, Dimensions, Platform, StyleSheet } from 'react-native';
...
const NoticeBar = ({ type = 'default', mode = '', text, direction = 'left', delay = 1000, scrollable: _scrollable = false, speed = 40, rightAreaText = '', leftIconKid = '', backgroundColor = '', color = '', borderRadius = 0, wrapable = false, onPressRightArea = () => { }, onPress = () => { }, renderLeftArea, renderRightArea, horizontalSpacing = rem(100) }) => {
    let verticalTimer = useRef(null); // 垂直滚动的定时器
    const [state, setState] = useState({
        show: true,
        showHorizontalClone: false,
    ...
    const textRef = useRef(null); // 横向文字ref，用来获取文字宽度
    const verticalScrollListRef = useRef([]);
    ...
    const verticalAnimation = (distance) => {
    ...
    const horizontalAnimation = ({ distance, horizontalDuration, }) => {
        const { timing, loop } = Animated;
        loop(timing(translateX, {
            toValue: -distance,
    ...
    const [innerScrollable, setInnerScrollable] = useState(undefined);
    ...
    const initHorizontalAnimation = () => {
        ...
        //@ts-expect-error
        (_a = textRef === null || textRef === void 0 ? void 0 : textRef.current) === null || _a === void 0 ? void 0 : _a.measure((x, y, width) => {
            ...
            setState(Object.assign(Object.assign({}, state), { horizontalTextWidth,
                horizontalDuration,
                showHorizontalClone }));
            if (direction === 'left') {
    ...
    const onPressRightBox = () => {
        if (mode === 'closeable') {
            setState(Object.assign(Object.assign({}, state), { show: false }));
        }
    ...
    const LeftArea = () => {
        return (<View style={[styles.leftIcon]}>
                <KidIcon kid={leftIconKid} size={icon_size} style={{ tintColor: color ? color : text_color[type] }}/>
            </View>);
    };
    const RightArea = () => {
        return (<TouchableOpacity activeOpacity={1} onPress={onPressRightBox} style={styles.rightLink} testID="notice-bar-right-area">
                {!!rightText && (<Text style={[styles.contentTextStyle, { color: color ? color : text_color[type] }]}>
                        {rightText}
                    </Text>)}
                {!!rightIcon && (<KidIcon kid={rightIcon} size={icon_size} style={{ tintColor: color ? color : text_color[type] }}/>)}
            </TouchableOpacity>);
    };
    ...
    const ContentArea = () => {
        ...
        if (!_scrollable ||
            ...
            return (<TouchableOpacity activeOpacity={1} onPress={onPressContent}>
                    <Text numberOfLines={wrapable ? 0 : 1} style={[styles.contentTextStyle, { color: color ? color : text_color[type] }]}>
            ...
            return (<TouchableOpacity activeOpacity={1} onPress={onPressContent}>
                ...
                <View style={{
                        width: horizontalSpacing,
                            ...
                            {!!state.showHorizontalClone && (<Text style={[
                        styles.contentTextStyle,
                        { color: color ? color : text_color[type] },
                        styles.scrollContentTextStyle,
                    ]}>
        ...
        if ((direction === 'up' || direction === 'down') && verticalScrollList.length > 1) {
            return (<TouchableOpacity activeOpacity={1} onPress={onPressContent}>
                    ...
                    transform: [{ translateY: translateY }],
                            ...
                            {verticalScrollList.map((item, index) => (<View style={[styles.verticalScrollItem]} key={index}>
                                    <Text style={[
                        styles.contentTextStyle,
                        { color: color ? color : text_color[type] },
                        {

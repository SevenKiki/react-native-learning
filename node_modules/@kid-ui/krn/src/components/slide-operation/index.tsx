...
import { ActionItemType, ActionType, SlideOperationProps} from './types';
import { useStyle } from './utils';

const SlideOperation: React.FC<SlideOperationProps> = ({
   rootTag,
   customStyle={},
   enableLeftSlideBack = true,
   actionWidth=88,
   leftActions=[],
   rightActions = [],
   leftActionWidth,
   rightActionWidth,
   children,
   canSlideBackSpace = 11,
   index=-1,
   onCallBack,
   close = false,
   closeAfterClickedBtn=true,
   numberOfLines = 1,
   ellipsizeMode = 'clip'
}) => {
     ...
     */
    const handleAction = (position: number, animateType: 'timing' | 'spring') => {
     ...
     * @param gestureState
     */
    const handleMoveShouldSetPanResponderCapture = (
     ...
     * 渲染Action
     */
    const renderAction = ({ item, index, actionWidth }: ActionItemType) => {
        return (
            <TouchableOpacity
                testID={`ActionTouchableOpacity${index}`}
                key={index}
                style={[
                    styles.action,
                    {
                ...
                activeOpacity={
                    customStyle?.actionActiveOpacity || 0.5
                }
            >
                {item.render ? (
                    item.render()
    ...
    return (
        <View style={styles.slideContainer}>
            <View style={[styles.actionContainer]}>
                ...
                >
                    {leftActions.map((item: ActionType, index: number) => {
                        return renderAction({
                            item,
                            index,
                ...
                >
                    {rightActions.map((item: ActionType, index: number) => {
                        return renderAction({
                            item,
                            index,
